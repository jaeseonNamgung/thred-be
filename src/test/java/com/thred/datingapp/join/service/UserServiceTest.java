...
class UserServiceTest {
    ...
    void givenExistingMember_whenJoining_thenThrowsAlreadyJoinError() {
        ...
        CustomException exception = assertThrows(CustomException.class, () ->
                sut.join(joinUserRequest, joinDetailsRequest, multipartFile, List.of(multipartFile)));
        // then
    ...
    void givenValidUserInfo_whenJoining_thenReturnsUserId() {
      ...
      // then
        ...
        then(userDetailRepository).should().save(any(UserDetail.class));
        then(pictureRepository).should().saveAll(any());
    ...
    void givenInValidEmail_whenGetExistingUser_thenThrowsNotExistUserError() {
      // given
        ...
        JoinDetailsRequest joinDetailsRequest = createJoinDetailsRequest();
        MockMultipartFile mainProfile = new MockMultipartFile(
            "mainProfile", "new-main.jpg", "image/jpeg", "image-data".getBytes()
        );

        MockMultipartFile profile1 = new MockMultipartFile(
            "profile1", "profile1.jpg", "image/jpeg", "profile-data-1".getBytes()
        );
        MockMultipartFile profile2 = new MockMultipartFile(
            "profile2", "profile2.jpg", "image/jpeg", "profile-data-2".getBytes()
        );
    ...
    void givenValidRequest_whenRejoin_thenReturnsVoid() {
      // given
        ...
        given(userDetailRepository.findByUserId(anyLong())).willReturn(Optional.of(UserFixture.createDetails1(user)));
        given(pictureRepository.findByProfileIds(anyList())).willReturn(createPictures(2));
        given(userRepository.findAdminByEmailAndRole(anyString(), any(Role.class))).willReturn(Optional.of(admin));
        // when
        sut.rejoin(rejoinUserRequest, joinDetailsRequest, true, mainProfile, List.of(1L, 2L, 3L), newProfiles);
        // then
        ...
        then(userDetailRepository).should().findByUserId(anyLong());
        then(pictureRepository).should().findByProfileIds(anyList());
        then(pictureRepository).should().deleteProfileByIds(anyList());
        then(pictureRepository).should().saveAll(anyList());
        then(questionRepository).should().deleteById(anyLong());
    ...
    void givenValidUserIdAndRequest_whenSetBlockNumber_thenReturnVoid() {
        ...
        then(blockRepository).should(times(5)).save(any(Block.class));
    }

    private static List<BlockInfoRequest> createBlocks() {
        return List.of(
            new BlockInfoRequest("김민수", "010-1234-5678"),
            new BlockInfoRequest("이영희", "010-2345-6789"),
            new BlockInfoRequest("박준형", "010-3456-7890"),
            new BlockInfoRequest("최지우", "010-4567-8901"),
    ...
    public static JoinUserRequest createJoinUserRequest() {
        return new JoinUserRequest(
            1L,
                ...
                "010-1234-5678",
                "CODE123",
                PartnerGender.OTHER.getGender(),
                "fcm-token-value"
        );
    }
    private static RejoinUserRequest createRejoinUserRequest() {
        return new RejoinUserRequest(
            "test@example.com",
            Gender.MALE.getGender(),
            "테스트유저",
            "서울시",
    ...
    public static JoinDetailsRequest createJoinDetailsRequest() {
        return new JoinDetailsRequest(
                175,
                Drink.ENJOY.getDrink(),
                Smoke.NONE.getSmoke(),
                Belief.NON_RELIGIOUS.getBelief(),
                OppositeFriends.A_LITTLE_BIT.getAmount(),
                Job.OFFICE_WORKER.getJob(),
                Mbti.ESFJ.getMbti(),
                70
        );
    }
