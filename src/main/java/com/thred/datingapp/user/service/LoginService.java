...
public class LoginService {
  ...
  public OAuthLoginResponse loginWithOAuth(final OAuthLoginRequest loginRequest) {
    ...
    if (optionalUser.isEmpty()) {
      ...
      return OAuthLoginResponse.ofFailure();
    }

    User user = optionalUser.get();
    if (user.getCertification() == null || !user.getCertification()) {
      log.warn("[loginWithOAuth] 인증되지 않은 회원입니다. ===> userId: {}", user.getId());
      return OAuthLoginResponse.ofAuthorizationFailure(user.getId(), user.getRole().getRole());
    }

    verifyOAuthUserLoginState(user);
    ...
    return OAuthLoginResponse.ofSuccess(user.getId(), user.getRole().getRole(), accessToken, refreshToken);
  }
  ...
  public LoginResponse getLoginResponse(Long userId) {
    ...
    if (user.getCertification() == null || !user.getCertification()) {
      log.warn("[getLoginResponse] 인증되지 않은 회원입니다. ===> userId: {}", userId);
      return new LoginResponse(true, false, user.getRole().getRole());
    }
    log.info("[getLoginResponse] getLoginResponse 프로세스 완료 (인증된 회원) ===> userId: {}", userId);
    return new LoginResponse(true, true, user.getRole().getRole());
  }

  private void verifyOAuthUserLoginState(User user) {
